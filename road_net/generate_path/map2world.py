...
def interpolate(input_points, interVal=0.1):
	output_points = []
...
def calPointsInfo(points):
	...
	points = interpolate(points,3.0)#插值
#	points = generateMaxSpeed(points,1.5)
	return points
	
def translationPoints(points):
	points = rotation(points,1.3) #旋转
	delta_x = -0.2
	delta_y = 2.0
	points = offsetPoints(points, 288477.894+delta_x, 3496772.181+delta_y) #平移
